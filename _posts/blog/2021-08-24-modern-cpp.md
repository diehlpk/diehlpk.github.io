...
Many applied mathematics codes are based on the C++ programming language. However, many of these codes do not utilize modern C++ features, especially the features for the recent C++ 17 standard for parallel computations. These features can significantly simplify parallel computations using multiple cores. This review briefly introduces asynchronous programming introduced in the C++ 11 standard and the parallel algorithms introduced in the C++ 17 standard. With these two paradigms, the C++ code can be accelerated using multiple cores using the C++ standard without any need for some external application interface, e.g. OpenMP. As an outlook, we introduce the C++ standard library for parallelism and concurrency (HPX) [2], which implements some proposed features of the upcoming C++ 20 standard.

## Asynchronous programming 

Another aspect of parallel programming in C++ is asynchronous programming which was introduced with the C++ 11 Standard [4]. Here, the concept of futurization using `std::future` was introduced.  The future is a result of the asynchronous function launch using \lstinline{std::async} and is a placeholder for the result of the computation. The promise is made that once the computation has finished, the future contains the result. One can use the function `.get()` to access the result. If the computation has finished the result will be returned. If the computation is still on going, the function will block until it is done. Let, us look at the following Taylor expansion for the natural logarithm

$$ \ln(x) = \sum\limits_{n=1}^N \frac{(-1)^{n+1}}{n}(x-1)^n \textbf{.} $$

Listing 1 shows the asynchronous computation of the Taylor series. First, a function `taylor_part` which computes a part of the sum is defined in Line 5. In Line 20 the sum of length $n$ is separated into two partitions of $n/2$ and each of them is asynchronously launched, and a future is returned. Since these two function launches run asynchronously on two cores, we need to synchronize their results, and collect the results. In Line 25 the `.get()` function is called to access the result of each future. Note that the code will introduce a barrier and waits until the first and second future are ready.
...
2. Hartmut Kaiser, Patrick Diehl, Adrian S Lemoine, Bryce Adelstein Lelbach, Parsa Amini, Agustı́n Berge, John Biddiscombe, Steven R Brandt, Nikunj Gupta, Thomas Heller, et al. HPX-the c++ standard library for parallelism and concurrency. Journal of Open Source Software, 5(53):2352, 2020.
3. Chuck Pheatt. Intel® threading building blocks. J. Comput. Sci. Coll., 23(4):298, April 2008.
4. The C++ Standards Committee. ISO International Standard ISO/IEC 14882:2011, Programming Language C++. Technical report, Geneva, Switzerland: International Organization for Standardization (ISO)., 2011. [http://www.open-std.org/jtc1/sc22/wg21](http://www.open-std.org/jtc1/sc22/wg21).
5. The C++ Standards Committee. ISO International Standard ISO/IEC 14882:2017, Programming Language C++. Technical report, Geneva, Switzerland: International Organization for Standardization (ISO)., 2017. [http://www.open-std.org/jtc1/sc22/wg21](http://www.open-std.org/jtc1/sc22/wg21).